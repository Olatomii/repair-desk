...
const allowedActionsByRole: Record<
  "CLIENT" | "ARTISAN" | "OPERATOR",
  readonly string[]
> = {
...
export async function POST(
  ...
  const role =
    (session.user as typeof session.user & { role?: "CLIENT" | "ARTISAN" | "OPERATOR" })
      .role ?? "CLIENT";
  ...
  try {
    ...
    switch (parsed.data.action) {
      ...
      case "MARK_WORK_COMPLETE":
        booking = await markWorkComplete({
          bookingId,
          artisanUserId: session.user.id,
          actorId: session.user.id,
        });
    ...
    return NextResponse.json(
      { error: "Unable to complete the booking action." },
      { status: 500 },
    );
  }
}
