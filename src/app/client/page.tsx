            ...
            <article key={booking.id} className="card p-6">
              <div className="grid gap-4 md:grid-cols-[1fr_auto]">
                <div>
                  ...
                  <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#64706a]">{booking.problemDescription}</p>
                </div>
                <div className="md:text-right">
                  <span className="inline-flex rounded-full bg-[#edf3ef] px-3 py-1 text-xs font-black text-[#1f5b45]">{booking.status.replaceAll("_", " ")}</span>
                  <p className="mt-3 text-sm text-[#64706a]">{booking.artisan ? booking.artisan.user.name : "Awaiting artisan assignment"}</p>
                </div>
              </div>
              ...
              {booking.status === "REQUESTED" || booking.status === "ASSIGNED" ? (
                ...
                <EvidenceUpload bookingId={booking.id} kinds={["DOCUMENT"]} />
              ) : null}

              {booking.events.length > 0 ? (
                <details className="mt-5">
                  <summary className="cursor-pointer text-sm font-black text-[#1f5b45]">Recent activity</summary>
                  <ol className="mt-3 space-y-2 border-l border-[#d9ded8] pl-4 text-sm text-[#64706a]">
                    {booking.events.map((event) => (
                      <li key={event.id}><strong className="text-[#1e2522]">{event.type.replaceAll("_", " ")}</strong>{event.note ? ` — ${event.note}` : ""}</li>
                    ))}
                  </ol>
                </details>
              ) : null}
